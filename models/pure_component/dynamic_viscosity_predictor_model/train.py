...
def load_raw_data():
    ...
    df = pd.read_csv('data/dynamic_viscosity.csv')
...
def prepare_training_data():
    ...
    y_raw = df_valid["dynamic_viscosity"].values

# Log-transform target (base-10)
    y = np.log10(y_raw)
...
def train_and_save_model(n_trials=100, 
                         output_dir='dynamic_viscosity_model/artifacts'):
    """
    ...
    study = optuna.create_study(
        ...
        study_name="extratrees_dynamic_viscosity_tuning"
    )
    ...
    # Evaluate
    train_pred_log = final_model.predict(X_selected)

    train_rmse_log = np.sqrt(np.mean((y - train_pred_log) ** 2))
    train_mae_log = np.mean(np.abs(y - train_pred_log))

    print(f"Train RMSE (log10): {train_rmse_log:.4f}")
    print(f"Train MAE  (log10): {train_mae_log:.4f}")
    plt.scatter(y, train_pred_log, alpha=0.5)
    plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--')
    plt.xlabel("True log10(viscosity)")
...
class DynamicViscosityPredictor:
    """
    Simple predictor class for DynamicViscosity number prediction.
    
    Usage:
        predictor = DynamicViscosityPredictor()
        dynamic_viscosity_values = predictor.predict(["CCCCCCCC", "CC(C)C"])
    """
    
    def __init__(self, model_path="dynamic_viscosity_model/artifacts/model.joblib", 
                 selector_path="dynamic_viscosity_model/artifacts/selector.joblib"):
        """Load the trained model and feature selector."""
        print("Loading DynamicViscosity Predictor...")
    ...
    def predict(self, smiles_list):
        """
        Predict DynamicViscosity numbers for SMILES strings.
        
        Args:
            smiles_list: Single SMILES string, list of SMILES, or pandas Series
        
        Returns:
            List of predicted dynamic_viscosity values
        """
        ...
        # Predict
        predictions_log = self.model.predict(X_selected)

        # Convert back to linear viscosity
        predictions = np.power(10.0, predictions_log)
    ...
    def predict_with_details(self, smiles_list):
        ...
        if X_full is None:
            print("⚠ Warning: No valid molecules found!")
            return pd.DataFrame(columns=["SMILES", "Predicted_dynamic_viscosity", "Valid"])
        ...
        df_valid["Predicted_dynamic_viscosity"] = predictions

        df_valid["Valid"] = True
        
        # Mark invalid molecules
        all_results = pd.DataFrame({"SMILES": smiles_list})
        all_results = all_results.merge(df_valid[["SMILES", "Predicted_dynamic_viscosity", "Valid"]], 
                                        on="SMILES", how="left")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "train":
        ...
        model_path = "dynamic_viscosity_model/artifacts/model.joblib"
        selector_path = "dynamic_viscosity_model/artifacts/selector.joblib"
        ...
        if not os.path.exists(model_path) or not os.path.exists(selector_path):
            ...
            print("\nThis will create:")
            print("  - dynamic_viscosity_model/artifacts/model.joblib")
            print("  - dynamic_viscosity_model/artifacts/selector.joblib")
            sys.exit(1)
        ...
        print(f"Octane dynamic_viscosity: {predictions[0]:.2f}")
        print(f"Isobutane dynamic_viscosity: {predictions[1]:.2f}")
        ...
        print("\nIn your code:")
        print("  from train import dynamic_viscosity_predictor_model")
        print("  predictor = DynamicViscosityPredictor()")
