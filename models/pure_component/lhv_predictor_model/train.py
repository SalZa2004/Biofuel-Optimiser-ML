...
def train_and_save_model(n_trials=100, 
                         output_dir='lhv_model/artifacts'):
    """
    ...
    study = optuna.create_study(
        ...
        study_name="extratrees_lhv_tuning"
    )
...
class LHVPredictor:
    """
    Simple predictor class for LHV number prediction.
    
    Usage:
        predictor = LHVPredictor()
        lhv_values = predictor.predict(["CCCCCCCC", "CC(C)C"])
    """
    
    def __init__(self, model_path="lhv_model/artifacts/model.joblib", 
                 selector_path="lhv_model/artifacts/selector.joblib"):
        """Load the trained model and feature selector."""
        print("Loading LHV Predictor...")
    ...
    def predict(self, smiles_list):
        """
        Predict LHV numbers for SMILES strings.
        
        Args:
            smiles_list: Single SMILES string, list of SMILES, or pandas Series
        
        Returns:
            List of predicted lhv values
        """
    ...
    def predict_with_details(self, smiles_list):
        ...
        if X_full is None:
            print("⚠ Warning: No valid molecules found!")
            return pd.DataFrame(columns=["SMILES", "Predicted_lhv", "Valid"])
        ...
        df_valid["Predicted_lhv"] = predictions
        df_valid["Valid"] = True
        
        # Mark invalid molecules
        all_results = pd.DataFrame({"SMILES": smiles_list})
        all_results = all_results.merge(df_valid[["SMILES", "Predicted_lhv", "Valid"]], 
                                        on="SMILES", how="left")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "train":
        ...
        model_path = "lhv_model/artifacts/model.joblib"
        selector_path = "lhv_model/artifacts/selector.joblib"
        ...
        if not os.path.exists(model_path) or not os.path.exists(selector_path):
            ...
            print("\nThis will create:")
            print("  - lhv_model/artifacts/model.joblib")
            print("  - lhv_model/artifacts/selector.joblib")
            sys.exit(1)
        ...
        print(f"Octane lhv: {predictions[0]:.2f}")
        print(f"Isobutane lhv: {predictions[1]:.2f}")
        ...
        print("\nIn your code:")
        print("  from train import lhv_predictor_model")
        print("  predictor = LHVPredictor()")
