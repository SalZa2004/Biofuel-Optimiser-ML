"""
Test the trained YSI prediction model on a held-out test set.
"""
...
def load_and_split_data(test_size=0.2, random_state=42):
    ...
    # Clean data
    df.dropna(subset=["ysi", "SMILES"], inplace=True)
    
    # Remove outliers (same as training)
    df = df[(df["ysi"] >= 0) & (df["ysi"] <= 150)]
    
    print(f"Total samples: {len(df)}")
    print(f"ysi range: [{df['ysi'].min():.1f}, {df['ysi'].max():.1f}]")
...
def evaluate_model(predictor, test_df):
    ...
    if len(predictions) != len(test_df):
        ...
        results = results.merge(test_df[["SMILES", "ysi"]], on="SMILES", how="left")
        results = results[results["Valid"] == True]
        
        y_true = results["ysi"].values
        y_pred = results["Predicted_ysi"].values
    else:
        y_true = test_df["ysi"].values
...
def evaluate_training_set(predictor, train_df):
    ...
    if len(predictions) != len(train_df):
        ...
        results = results.merge(train_df[["SMILES", "ysi"]], on="SMILES", how="left")
        results = results[results["Valid"] == True]
        
        y_true = results["ysi"].values
        y_pred = results["Predicted_ysi"].values
    else:
        y_true = train_df["ysi"].values
        y_pred = np.array(predictions)
...
def plot_results(y_true, y_pred, save_path="evaluation_plots.png"):
    ...
    ax1.set_xlabel('Actual YSI', fontsize=12)
    ax1.set_ylabel('Predicted YSI', fontsize=12)
    ax1.set_title('Predicted vs Actual YSI Number', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    ax2.set_xlabel('Predicted YSI', fontsize=12)
...
def save_predictions(test_df, y_true, y_pred, save_path="test_predictions.csv"):
    ...
    results_df["Actual_YSI"] = y_true
    results_df["Predicted_YSI"] = y_pred
