...
def train_and_save_model(n_trials=100, 
                         output_dir='ysi_model/artifacts'):
    """
    ...
    study = optuna.create_study(
        ...
        study_name="extratrees_ysi_tuning"
    )
...
class CetanePredictor:
    ...
    Usage:
        ...
        ysi_values = predictor.predict(["CCCCCCCC", "CC(C)C"])
    """
    
    def __init__(self, model_path="ysi_model/artifacts/model.joblib", 
    ...
    def predict_with_details(self, smiles_list):
        ...
        all_results = all_results.merge(df_valid[["SMILES", "Predicted_YSI", "Valid"]], 
                                        on="SMILES", how="left")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "train":
        ...
        model_path = "ysi_model/artifacts/model.joblib"
        selector_path = "ysi_model/artifacts/selector.joblib"
        ...
        if not os.path.exists(model_path) or not os.path.exists(selector_path):
            ...
            print("\nThis will create:")
            print("  - ysi_model/artifacts/model.joblib")
            print("  - ysi_model/artifacts/selector.joblib")
            sys.exit(1)
        ...
        print(f"Octane ysi: {predictions[0]:.2f}")
        print(f"Isobutane ysi: {predictions[1]:.2f}")
